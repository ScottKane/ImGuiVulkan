...
public class ImGuiVulkanApplication
{
    ...
    private unsafe void DrawFrame(double delta)
    {
        ...
        uint imageIndex;
        var result = _vkSwapchain.AcquireNextImage(_device, _swapchain, ulong.MaxValue, _imageAvailableSemaphores[_currentFrame], default, &imageIndex);
    ...
    {
        foreach (var framebuffer in _swapchainFramebuffers) _vk.DestroyFramebuffer(_device, framebuffer, null);
    ...
    private unsafe void CreateInstance()
    {
        ...
        for (var i = 0; i < extCount; i++) newExtensions[i] = extensions[i];
    ...
    private void PickPhysicalDevice()
    {
        ...
        if (!devices.Any()) throw new NotSupportedException("Failed to find GPUs with Vulkan support.");
    ...
    private unsafe SwapChainSupportDetails QuerySwapChainSupport(PhysicalDevice device)
    {
        ...
        if (presentModeCount != 0)
        {
            ...
            for (var i = 0; i < presentModeCount; i++) details.PresentModes[i] = modes[i];
        }
    ...
    private unsafe QueueFamilyIndices FindQueueFamilies(PhysicalDevice device)
    {
        ...
        for (var i = 0u; i < queryFamilyCount; i++)
        {
            ...
            if (presentSupport == Vk.True) indices.PresentFamily = i;
    ...
    {
        var vertShaderCode = Resources.LoadEmbeddedBytes($"{nameof(ImGuiVulkan)}.shader.vert.spv");
        var fragShaderCode = Resources.LoadEmbeddedBytes($"{nameof(ImGuiVulkan)}.shader.frag.spv");
    ...
    private unsafe ShaderModule CreateShaderModule(byte[] code)
    {
        ...
        fixed (byte* codePtr = code) createInfo.PCode = (uint*) codePtr;
    ...
    {
        public static byte[] LoadEmbeddedBytes(string path)
        {
